...
`combined` is the default branch and normal personal build. The releases shown
in this repository's sidebar are built from `combined`, so they contain both
Salvium support and the personal donation layer. Separately tagged SAL-only
artifacts must be built from `salvium` and retain upstream donation behavior.
See [the branching guide](docs/BRANCHING.md) for the complete maintenance,
integration, build, and release workflow.
...
* **[Latest combined Salvium binary release](https://github.com/mysalvium/xmrig_salvium/releases/latest)** — rebuilt after every successful push to `combined` and includes the personal donation layer
* **[Combined source ZIP](https://github.com/mysalvium/xmrig_salvium/archive/refs/heads/combined.zip)** — the default branch with both custom layers
* **[Build history and weekly artifacts](https://github.com/mysalvium/xmrig_salvium/actions/workflows/deploy.yml)**
